...
export type PacketLossMode = "combined" | "split";

export type PacketLossDetailKey = "p5" | "p15";

export type PacketLossSiteScope = "region" | "area";

/** Baris detail site: kolomnya mengikuti response, jadi dibiarkan dinamis. */
export type DailyMonitoringSiteRow = Record<string, unknown>;

export interface DailyMonitoringSiteParams {
  type: PacketLossSiteScope;
  value: string;
  pl?: PacketLossDetailKey;
}

export interface DailyMonitoringDownloadParams {
  type?: PacketLossSiteScope;
  value?: string;
  pl?: PacketLossDetailKey;
}

export interface DailyMonitoringDownloadFile {
  blob: Blob;
  fileName: string;
}
