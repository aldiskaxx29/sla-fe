...
import type { HistorySlaTrendPoint } from "@/app/types/first-insight/historySla.types";
  ...
  const rows = point.breakdown
    .map(
      ...
      <div style="display:flex;justify-content:space-between;gap:24px;font-size:11px;color:#334155;line-height:18px">
        ...
        <span style="color:#020617">${escapeHtml(item.value)}</span>
      </div>`,
    )
    .join("");
  ...
  return `
    <div style="min-width:132px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:6px">
        <span style="font-size:12px;font-weight:600;color:#020617">${escapeHtml(point.period)}</span>
        <span style="border-radius:6px;background:#fdeeee;padding:1px 6px;font-size:11px;font-weight:600;color:#dc2626">${escapeHtml(point.total)} KPI</span>
      </div>
      ${rows}
    </div>`;
};
...
export function HistorySlaTrendChart({
      ...
      series: [
        {
          ...
          data: points.map((point) => ({
            value: point.total,
            symbolSize: point.month === activeMonth ? 10 : 7,
          })),
          symbol: "circle",
          symbolSize: 7,
  ...
  return (
    <SectionCard className="flex min-w-0 flex-col gap-2 p-4">
      ...
      {loading ? (
        ...
        <div className="flex h-[250px] items-center justify-center text-sm text-[#64748b]">
          {error ? "Gagal memuat trend KPI." : "Data trend belum tersedia."}
        </div>
      ) : (
